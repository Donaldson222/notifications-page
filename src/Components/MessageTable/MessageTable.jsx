...
const MessageTable = ({ searchTerm, filter }) => {
  ...
  const filteredData = data.filter((row) => {
    ...
    const filterMatch =
      filter === "All" ||
      (filter === "Unread" && row.status === "Unread") ||
      (filter === "Read" && row.status === "Read") ||
      (filter === "Fuel" && row.messageType === "Fuel");
  ...
  return (
    <TableContainer
      style={{ backgroundColor: "white", borderRadius: "8px", overflow: "hidden", marginTop: "20px", marginLeft: "20px", marginRight: "20px" }}
    >
      <Table>
        <TableHead>
